...
select count(case when gender='M' then 1 end) as '남',count(case when gender='F' then 1 end) as '여' from employees;
select count(*) from employees group by gender;
-- 문제4.현재 근무하고 있는 직원 수는 몇 명입니까? (salaries 테이블을 사용합니다.) 
select  count(*) as '현재근무인원' from salaries where to_date='9999-01-01';
-- 문제5.부서는 총 몇 개가 있나요?
select  count(distinct dept_no) as '부서 수' from departments;
-- 문제6.현재 부서 매니저는 몇 명이나 있나요?(역임 매너저는 제외)
select  count(*)as '현 부서 매니저 수' from dept_manager where to_date='9999-01-01';
...
select count(*) as '급여120,000이상' from salaries where to_date='9999-01-01' and salary>120000;
